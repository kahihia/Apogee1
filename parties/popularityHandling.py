...
def lottery_popularity_join(party_obj):
	...
	party_obj.interaction_pts = F('interaction_pts') + (join_val*lottery_mult)
...
def lottery_popularity_star(party_obj):
	...
	party_obj.interaction_pts = F('interaction_pts') + (star_val)
	party_obj.popularity = math.log10(new_points) + F('time_pts')
	party_obj.save2(update_fields=['interaction_pts', 'popularity'])
# adds popularity
...
def bid_popularity_join(party_obj):
	...
	party_obj.popularity = math.log10(new_points) + F('time_pts')
	party_obj.interaction_pts = F('interaction_pts') + (join_val*bid_mult)
	party_obj.save2(update_fields=['interaction_pts', 'popularity'])
# adds popularity
...
def buyout_popularity_join(party_obj):
	...
	party_obj.popularity = math.log10(new_points) + F('time_pts')
	party_obj.interaction_pts = F('interaction_pts') + (join_val*buyout_mult)
	party_obj.save2(update_fields=['interaction_pts', 'popularity'])
# adds popularity
